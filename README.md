...
**Serving an LLM to many people at once from a single 4 GiB laptop GPU â€”
...
auth and token budgets â€” with Prometheus, Grafana, ten predictive alert rules,
...
thermally throttles from ~1,500 MHz to ~950â€“1,200 MHz sustained. The
constraints are severe enough that nothing could be assumed, so everything was
measured â€” and most of the interesting numbers contradicted a prediction that
...
users at ~75% (usable)                                â‰ˆ 22
```

## The envelope â€” what "22 users" actually bounds
...
| --- | --- | --- |
| **Concurrency** | 6 requests in flight at the engine â€” the gateway's limit | The engine's own `--max-num-seqs` is 32, deliberately above this, so it never binds first |
| **Think time** | 12 s median (log-normal). Real users often think longer | 30 s â†’ ~49 users; 45 s â†’ ~72. It scales with `(think + service) / service` |
| **Output per turn** | 192 tokens in the simulator (~3 s of decode) | 512 tokens â†’ ~12 users; 1,024 â†’ ~8; 2,048 â†’ ~6. **Output length is the strongest lever after concurrency** |
...
| **Prompt size per request** | Chat-length, ~500â€“1,000 tokens | A single 20k-token prompt holds every short request for ~20 s â€” the one documented failure (case 3 below) |
| **Context window** | 32,768 tokens, hard (`rope_scaling: null`). Costs zero KV cache vs 4k â€” measured | Conversations that exceed it have their oldest turns dropped, reported in `X-Context-Trimmed-Messages`. Never a 400 |
| **Max output per call** | Gateway default **1,024** if the client sends none; **ceiling 2,048** (clamped, reported in a header) | Without this, vLLM defaults an absent `max_tokens` to *the rest of the window* â€” ~30k tokens, holding a slot for minutes |
...
| **Thermal state** | All numbers taken heat-soaked at 86â€“87 Â°C die temperature | A cold card runs up to ~40% faster for the first two minutes. Compare only runs from the same idle temperature |
...
## Three different numbers are called "concurrency" â€” here is which is which

| Number | What it is | Fixed or dynamic |
| ---: | --- | --- |
| **6** | **Requests the gateway lets onto the GPU at once.** The measured SLO ceiling. This is the one that matters | The *cap* is fixed (`GATEWAY_MAX_INFLIGHT`). What fits under it is dynamic: a long prompt costs up to 4 of the 6 slots, and each API key's share is `âŒˆ6 / contending keysâŒ‰` |
| **32** | `--max-num-seqs`: the engine's own batch ceiling | Fixed. Set above 6 on purpose so it can never be the binding limit |
| **2.13x** | vLLM's startup line "maximum concurrency for 32,768 tokens" â€” how many *full-window* sequences the 69,760-token KV pool holds | Not a limit at all. PagedAttention allocates 16-token blocks on demand, so ~95 typical chat sequences fit; it only describes the worst case |
...
leaving room for 2 short ones â€” 3 total â€” and a second 20k prompt is shed.
Measured `peak_inflight` in every run: 6. Measured engine `num_requests_running`
tracks it exactly, because nothing between the gateway and the engine limits.

Inside that cap the batch is fully dynamic â€” continuous batching recomposes it
every scheduler step as sequences finish and new ones are admitted.

**What happens as context grows** â€” three mechanisms at three scales, all measured:

| Scale | Mechanism | Measured |
| --- | --- | --- |
| Within one reply | KV blocks allocated on demand, 16 tokens at a time. If the 69,760-token pool fills, vLLM **preempts**: evicts a running sequence's blocks and re-prefills it later (V1 preempts by recompute; there is no CPU swap in V1) | **0 preemptions in every v2 run** â€” KV peaked at ~17%. v1 forced eviction deliberately and the gateway's accounting stayed exact (Boundary 4) |
| Between turns | The conversation's history sits in the prefix cache as free blocks, evicted LRU. On the next turn vLLM reuses the longest cached prefix; a miss re-prefills everything at ~9x the cost | Hit rate **74â€“97%** across every run. Prefill is quadratic, so the cost of a miss grows faster than the conversation does |
| At the window | Past 32,768 tokens the **gateway** drops the oldest non-system turns and reports it in `X-Context-Trimmed-Messages` | 95k tokens sent â†’ 27 messages dropped â†’ 200. Never a 400 |
...
| **Best within SLO** | 6 | **295â€“332 tok/s** | 471 in / 256 out, p95 TTFT 1,005â€“1,135 ms, +24â€“33% margin |
| **Under deep chassis heat-soak** | 4 | 188 tok/s | Same shape; card idled at ~78 Â°C before the run. The SLO ceiling drops to 4 in this state â€” the range is thermal, not noise |
| Single stream | 1 | 59â€“89 tok/s | Same shape; the spread is thermal |
| One giant prompt in flight | 3 | ~0 for others for ~20 s | A 20k-token prefill blocks everyone; the documented failure |
...
| --- | ---: | ---: | --- |
| **Host GPU power mode** â†’ *Prefer maximum performance* | 4.15 tok/s, card stuck in P8 | 47.99 tok/s, P0 | **11.6x**, zero code. Silent â€” no engine metric showed it |
| **Model: 3B-AWQ â†’ 1.5B-AWQ** | SLO holds to conc 2 @ 64.7 tok/s; 28k-token KV | conc 6 @ 295â€“332 tok/s; 69,760-token KV | **4.3x SLO-compliant throughput**, 2.4x KV cache. Decode âˆ weight bytes (âˆ’38%), prefill âˆ params (âˆ’51%), both predicted within 3% |
| **Shared system prompt** (prefix cache) | SLO fails at conc 4 | holds at conc 8 | **2x concurrency, TTFT âˆ’82%**, KV usage halved |
| **`--max-model-len` 4,096 â†’ 32,768** | 4k window | 32k window | **Zero KV cost** â€” 69,616 tokens at both. A ceiling, not an allocation |
| **`--max-num-seqs` 8 â†’ 32** | peak 367 tok/s | peak 686 tok/s | +87% peak, **0% SLO gain** â€” but the engine can no longer be a hidden limiter |
| **Admission control** (6 in flight) | TTFT 13 s and rising under overload | bounded | Unbounded â†’ bounded |
| **Queue timeout 2.0 â†’ 0.6 s** | flat at 2,070 ms, **49/58 users over SLO** | flat at 778 ms, **0/58 over** | Zero breaches for +10 pts shedding |
| **Per-key share: fixed â†’ dynamic** | one abuser takes 50%, 10/10 users over SLO | 14%, 0/10 over, 98.3% of abuse shed | Fairness that survives a real attacker |
...
| **Output bound** (default 1,024 / cap 2,048) | absent `max_tokens` â†’ ~30k-token hold | bounded ~30 s worst case | Slot hold time is finite |
...
Measured with `chat_sim.py` â€” Poisson arrivals, log-normal think time,
...
2.0 s queue timeout and produced an equally flat curve â€” but flat at 2,070 ms,
above the objective, with 49 of 58 users breaching. Worst-case TTFT â‰ˆ queue
...
| --- | --- | --- |
| **Prefix cache hit rate** | Falls *before* latency rises â€” spare capacity absorbs the extra prefill first | vLLM `/metrics`, scraped directly |
| **Admission queue depth** | A request has to wait before it can be slow | gateway `/metrics` |
| **KV cache %** and **preemptions** | 8â€“17% is normal here; 85% means something changed qualitatively | vLLM |
...
| **Sustained throughput below floor with a busy queue** | The only software-visible symptom of thermal throttling â€” stated as a proxy, because WSL2 gives Prometheus no GPU telemetry | vLLM + gateway |
...
**And when something does break** â€” the 14-case
[failure matrix](docs/FAILURE_MATRIX.md), each case with the behaviour
*expected before testing* next to the behaviour measured:

| Case | Measured |
| --- | --- |
| Engine crashes mid-stream | Detected in 1.4 s, auto-recovered in 83 s. Gateway `/health` stayed 200 throughout (never restarted), `/ready` went 503â†’200. **162 of 162 requests in the ledger, drift 0** |
| Gateway restarts under load | Back in 10.4 s; in-flight requests fail with a dropped connection, budgets intact |
| Client disconnects mid-stream | Slot released, usage recorded â€” from the generator's `finally` |
| Billing database unwritable | `200, 200, 200, 503â€¦` â€” three unbilled, then closed, then auto-recovered |
| One key at 50 concurrent | Normal users' p95: 191 â†’ 536 ms. **Still 64% inside the SLO.** 4,669 of 4,749 abusive requests shed |
| Burst of 60 simultaneous (10x limit) | 6 served at p95 263 ms, 54 shed with `Retry-After` |
| Conversation exceeds 32k | 95k tokens sent â†’ 27 oldest messages dropped â†’ engine saw 9.7k. HTTP 200 |
...
```text
  ...
  browser â”€â”€â”€â”€â”€â”€â”€â”€â–º â”‚  gateway  (FastAPI, :8080)               â”‚
  OpenAI client â”€â”€â–º â”‚                                          â”‚
                    â”‚   auth â”€â–º budget â”€â–º context policy â”€â–º    â”‚
                    â”‚   output bound â”€â–º   admission control â”€â” â”‚
                    â”‚                                        â”‚ â”‚
                    â”‚   /chat  /dashboard  /metrics  /ready   â”‚ â”‚
                    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”˜
                              â”‚                              â”‚
                    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”        â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
                    â”‚ SQLite            â”‚        â”‚ vLLM (:8000)             â”‚
                    â”‚ keys, budgets,    â”‚        â”‚ Qwen2.5-1.5B-AWQ         â”‚
                    â”‚ request ledger    â”‚        â”‚ awq_marlin Â· prefix cacheâ”‚
                    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜        â”‚ 69,760-token KV pool     â”‚
                                                 â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”                    â”‚
                    â”‚ Prometheus :9090  â”‚â—„â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                    â”‚ Grafana    :3000  â”‚â—„â”€â”€â”€ gateway /metrics
                    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```

**The gateway is the only limiter.** `--max-num-seqs` is set to 32 â€” far above
the gateway's 6 â€” specifically so the engine's own cap can never silently shape
...
**You need:** an NVIDIA GPU with â‰¥ 4 GiB VRAM and a driver providing **CUDA â‰¥ 12.8**
(the vLLM image refuses to start below that â€” checked before any of your code
...
take 60â€“90 s. `docker compose up -d` returns in about six seconds â€” that is not
...
| <http://localhost:3000> | Grafana â€” engine dashboard + capacity dashboard |
...
LLM decode and cost **11.6x** â€” silently. Fix: NVIDIA Control Panel â†’ Power
management mode â†’ *Prefer maximum performance*.
...
discarded warm-up, and records the GPU's idle temperature â€” because
...
starts vLLM â€” GitHub's runners have no GPU, and the numbers above cannot be
validated there.
...
plan as *"a testable prediction this project cannot test"* â€” and it got tested
by accident.

**2. Prefill is quadratic and everyone here assumed it was linear.** Fitting
three prompt sizes gives `TTFT â‰ˆ 0.13 + n/3070 + nÂ²Â·1.11e-8`, matching measured
...
The full list â€” **twenty-four wrong predictions and what each one taught** â€” is
in [docs/FINAL_REPORT.md Â§11](docs/FINAL_REPORT.md#11-what-this-project-got-wrong-and-what-that-taught).
...
| The raw chronological log â€” every stage, every bug, every correction | [PROGRESS_LOG.md](PROGRESS_LOG.md) |
...
* **Answer quality is barely measured.** On this project's control question the
  1.5B describes the KV cache as a generic key-value store â€” the same error the
